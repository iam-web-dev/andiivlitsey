...
const Info_section_main = ({ lang, setLang }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (socialData && socialData.results) {
          const links = socialData.results.reduce(
            (acc, item) => {
              if (item.name === "facebook") acc.facebook = item.url;
              if (item.name === "instagram") acc.instagram = item.url;
              if (item.name === "telegram" || item.name === "send")
                acc.telegram = item.url;
              return acc;
            },
  ...
  return (
    <div className="w-full sm:h-[60px] h-auto min-h-[40px] py-2 sm:py-0 bg-[#FFD859] xl:px-[110px] lg:px-[60px] md:px-[30px] px-[20px] flex items-center xl:justify-between lg:justify-between md:justify-center justify-center text-[#303030]">
      <div className="flex flex-col sm:flex-row gap-[5px] sm:gap-[20px] items-center">
        {/* Phone */}
        <a
      ...
      <div className="hidden lg:flex flex-row gap-[40px] items-center justify-center">
        ...
        <div className="flex flex-row items-center justify-center gap-[30px]">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="cursor-pointer hover:scale-[110%] active:scale-[95%] duration-300"
          >
        ...
        <div className="flex flex-row gap-[20px] text-[#303030]">
          <p
            ...
            className={`text-[18px] ${
              lang === "uz" ? "text-[#906e00] font-[600]" : "font-[400]"
            } cursor-pointer`}
          >
            O'z
          </p>
          <p
            ...
            className={`text-[18px] ${
              lang === "ru" ? "text-[#906e00] font-[600]" : "font-[400]"
            } cursor-pointer`}
          >
            Ру
          </p>
          <p
            ...
            className={`text-[18px] ${
              lang === "en" ? "text-[#906e00] font-[600]" : "font-[400]"
            } cursor-pointer`}
          >
            En
          </p>
