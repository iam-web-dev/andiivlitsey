...
const Footer_main = ({ lang }) => {
  ...
  const translations = {
    ...
    ru: {
      ...
      address: 'Адрес',
      email: 'Email',
      title: 'Академический лицей Андижанской области Министерства внутренних дел',
    ...
    en: {
      ...
      address: 'Address',
      email: 'Email',
      title: 'Academic Lyceum of Andijan Region under the Ministry of Internal Affairs',
      ...
      <div className="w-full px-[20px] md:px-[40px] lg:px-[60px] xl:px-0 xl:w-[1220px]">
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 xl:flex xl:flex-row gap-[30px] md:gap-y-[40px] xl:gap-[120px]">
          ...
          <div className="flex flex-col gap-[20px]">
            <h1 className="font-inter font-[600] text-[18px] sm:text-[20px] leading-[100%] text-[#FFFFFF]">{t.social}</h1>
          ...
          <div className="flex flex-col gap-[10px] sm:gap-[20px]">
            <h1 className="font-inter font-[600] text-[18px] sm:text-[20px] text-[#FFFFFF]">{t.contact}</h1>
            <h1 className="font-inter font-[400] text-[16px] sm:text-[18px] text-[#B5D3FF]">{contact.phone}</h1>
            {contact.phone2 && (
              <h1 className="font-inter font-[400] text-[16px] sm:text-[18px] text-[#B5D3FF]">{contact.phone2}</h1>
            )}
          </div>

          <div className="flex flex-col gap-[10px] sm:gap-[20px]">
            <h1 className="font-inter font-[600] text-[18px] sm:text-[20px] text-[#FFFFFF]">{t.address}</h1>
          ...
          <div className="flex flex-col gap-[10px] sm:gap-[20px]">
            <h1 className="font-inter font-[600] text-[18px] sm:text-[20px] text-[#FFFFFF]">{t.email}</h1>
            <a href={`mailto:${contact.email}`} className="font-inter font-[400] text-[16px] sm:text-[18px] text-[#B5D3FF] hover:underline">
              {contact.email}
            </a>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-start md:justify-between md:items-center mt-[40px] md:mt-[60px] xl:mt-[80px] gap-[20px] md:gap-[0px]">
          <h1 className="w-full md:w-[350px] xl:w-[283px] font-inter font-[700] text-[18px] sm:text-[20px] text-[#FFFFFF]">{t.title}</h1>
          <div
            onClick={() => window.open("https://t.me/nsd_corporation", "_blank")}
            className="flex flex-col text-white sm:items-end font-[600] text-[13px] cursor-pointer"
          >
            <p className="pl-[12px] md:pr-[13px]">Powered by</p>
            <img src={nsd_logo} className="w-[250px] h-auto" alt="NSD Corporation" />
          </div>
        </div>
